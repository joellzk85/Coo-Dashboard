...
export const Dashboard121Tracker: React.FC<Dashboard121TrackerProps> = ({ onOpenAdd121 }) => {
  ...
  // Filter 121s for the selected month and company
  const monthlySessions = sessions121.filter((s) => {
    const matchesMonth = s.monthYear === selectedMonth || s.date.startsWith(selectedMonth);
  ...
  const checkCadenceStatus = (dept: Department) => {
    ...
    // If weekly required, target is ~4 completed sessions per month; if monthly, target is at least 1.
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-500 font-medium">Selected Month: {selectedMonth}</span>
            </div>
      ...
      {/* 3. 121 Session History & Detailed Notes */}
      <section className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              <Users className="w-5 h-5 text-blue-600" />
              <span>1-on-1 Session Log & Notes ({monthlySessions.length})</span>
            </h3>
            <p className="text-xs text-slate-500 mt-0.5">
              Recorded notes, sentiment ups & downs, follow-up action items, and session energy ratings.
            </p>
          </div>
        ...
        {monthlySessions.length === 0 ? (
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-8 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500 font-medium">No 121 sessions logged for {selectedMonth} matching your filter.</p>
            <button
              onClick={onOpenAdd121}
