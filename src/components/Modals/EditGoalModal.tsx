        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 flex-1">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">
              Goal Impact Level <span className="text-slate-400 font-normal">(Weights in Automated Grading Engine)</span>
            </label>
            <select
          ...
          {/* Next Academy Specific Metric Fields */}
          {(formData.companyId === 'next_academy' || formData.academyRevenueTarget !== undefined) && (
            <div className="p-4 bg-slate-950/80 border border-amber-500/30 rounded-2xl space-y-3">
              <div className="flex items-center gap-2 text-amber-400 font-bold text-xs uppercase tracking-wider">
                <span>🎓 Next Academy Specific Metrics</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-semibold text-slate-300 mb-1">Current Revenue (RM)</label>
                  <input
                    type="number"
                    step="1000"
                    placeholder="e.g. 1250000"
                    value={formData.academyRevenueCurrent ?? ''}
                    onChange={e => setFormData({ ...formData, academyRevenueCurrent: e.target.value })}
                ...
                <div>
                  <label className="block text-[11px] font-semibold text-slate-300 mb-1">Target Revenue (RM)</label>
                  <input
                    type="number"
                    step="1000"
                    placeholder="e.g. 1500000"
                    value={formData.academyRevenueTarget ?? ''}
                    onChange={e => setFormData({ ...formData, academyRevenueTarget: e.target.value })}
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-amber-500"
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-slate-300 mb-1">Days of Training Conducted</label>
                  <input
                    type="number"
                    placeholder="e.g. 95"
                    value={formData.academyTrainingDaysCurrent ?? ''}
                    onChange={e => setFormData({ ...formData, academyTrainingDaysCurrent: e.target.value })}
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-amber-500"
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-semibold text-slate-300 mb-1">Target Training Days</label>
                  <input
                    type="number"
                    placeholder="e.g. 120"
                    value={formData.academyTrainingDaysTarget ?? ''}
                    onChange={e => setFormData({ ...formData, academyTrainingDaysTarget: e.target.value })}
                    className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-amber-500"
                  />
                </div>
              </div>
